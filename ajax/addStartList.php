...
foreach ($kategorieClass->getAllKategorie() as $option) {
    $kategorie .= '<option value="' . $option['id'] . '">' . $option['long'] .
             '</option>';
}
...
print '<div>Datum: <input type="text" value="' . date('d.m.Y') .
         '" id="datum"/></div>';
print 
        '<div>Čas startu prvního čísla: <input type="text" value="09:30:30" id="cas"/></div>';
