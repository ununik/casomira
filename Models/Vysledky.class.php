...
class Vysledky
{
	public function getResultForCategory($category, $mezicas=0, $lastIdNumber = 0)
	{
		...
		$result->execute(array(':kategorie' => $category, ':cisloMezicasu' => $mezicas));
		...
		for ($i=0; $i<count($results); $i++) {
			...
			$return .= '<td class="resultStartovniCislo">'.$results[$i]['startovniCislo'].'</td>';
			$return .= '<td>'.$results[$i]['jmeno'].'</td>';
			if ($i == 0) {
				$return .= '<td class="resultTime">'.date('H:i:s', $results[$i]['tracktime'] - 3600).'</td>';
			} else {
				$return .= '<td class="resultTime">+'.date('H:i:s', $results[$i]['tracktime'] - $results[0]['tracktime'] - 3600).'</td>';
			}
			$return .= '</tr>';
		}
		$return .= '</table>';
